...
#define half_of_PI 1.57079
	...
	// compression_params.push_back(2);  //这里设置保存的图像质量级别
...
{
    Coordination_pose(){x=0,y=0,z=0;angl.roll=0,angl.pitch=0,angl.yaw=0;};
    double x,y,z;
    EulerAngles angl;
};
...
Coordination_pose add_coordinations(const std::string &frame,const std::string &child_frame,const tf::TransformListener &tf_listen_,std::vector<cti_msgs::TabState> &coordinations)
{
    ...
    //监听两个坐标系之间的变换
    try{
        tf_listen_.waitForTransform( frame , child_frame , ros::Time(0), ros::Duration(0.01));
        tf_listen_.lookupTransform( frame , child_frame , ros::Time(0), transform_base);
    ...
    // position.pose.pose.orientation.w = transform_base.getRotation().getW();
    // position.pose.pose.orientation.z = transform_base.getRotation().getZ();
    // position.pose.pose.orientation.y = transform_base.getRotation().getY();
    // position.pose.pose.orientation.x = transform_base.getRotation().getX();
    ...
    stringstream ss;
    ss<<"yaw:"<<setPercision(Coordin_pose.angl.yaw)<<"  roll:"<<setPercision(Coordin_pose.angl.roll)<<"  pitch"<<setPercision(Coordin_pose.angl.pitch)<<"  x:"<<setPercision(Coordin_pose.x)<<"  y:"<<setPercision(Coordin_pose.y)<<"  z:"<<setPercision(Coordin_pose.z);
...
void draw_coordinations(cv::Mat &dst,const Coordination_pose &coordination_pose,const int &height,const int &width)
{
